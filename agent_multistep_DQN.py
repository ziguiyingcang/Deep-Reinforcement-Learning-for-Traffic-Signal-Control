...
class TestAgent():
    ...
    def get_action(self, ob):
        ...
        return action_idx'''
        q_values = self.model.predict([ob])
    ...
    def get_sample(self):
        sample_indices = random.sample(range(self.n_multi_step, len(self.memory)), self.batch_size)
        samples = []
        for j in range(self.batch_size):
            one_sample = []
            one_sample.append(self.memory[sample_indices[j]][0]) #ob
            one_sample.append(self.memory[sample_indices[j]][1]) #action
            #To calculate reward for Multi-step DQN
            reward_sum = 0.0
    ...
    def replay(self):
        ...
        minibatch = self.get_sample()
        obs, actions, rewards, next_obs, dones = [np.stack(x) for x in np.array(minibatch).T]
        target = rewards + self.gamma ** self.n_multi_step * np.amax(self.target_model.predict([next_obs]), axis=1)

        #To store Q value
        avg_q = str(np.mean(target))
        print("Average Q value is {}".format(avg_q))
        with open("avg_q_multistep_dqn.txt", 'a', encoding = 'utf-8') as f:
            f.write(avg_q + "\n")
        f.close()
        ...
        with open("loss_multistep_dqn.txt", 'a', encoding = 'utf-8') as x:
            x.write(train_loss + "\n")
        x.close()
