    ...
    for e in range(args.episodes):
        ...
        while i < args.steps:
            if i % args.action_interval == 0:
                ...
                actions = agent.act_(observations_for_agent, e)
            ...
            if e < agent.para_noise_max_ep and total_decision_num % agent.update_param_fq == agent.update_param_fq - 1:
                ...
                for ob in obs:
                    unperturbed_actions.append(agent.get_action(ob, False, False, True))
                unperturbed_actions = np.asarray(update_param_batch)
                perturbed_actions = np.asarray([data[1] for data in update_param_batch])
...
def run_simulation(agent_spec, simulator_cfg_file, gym_cfg,metric_period,scores_dir,threshold):
    ...
    while not done:
        ...
        actions = agent.act(all_info, step)
