...
class Memory(object):
	...
	def reme(self, ob, action, reward, next_ob, done, action_probs):
		experience = (ob, action, reward, next_ob, done, action_probs)
...
class DDPGAgent(object):
	def __init__(self):
		...
		self.ou_noise_max_ep = 95
		self.para_noise_max_ep = 95
	...
	def perturb_actor_parameters(self):
		...
		for para in net:
			...
			#print("type of para.get_shape().as_list() is {}".format(type(para.get_shape().as_list())))
			#print("para is {}".format(para))
			shape = para.get_shape().as_list()
			#tf.cast(shape, tf.int32)
			#print("shape is {}".format(shape))
			para = para + tf.random.normal(shape, dtype = tf.float32) * self.ap_explore.current_stddev
			new_net.append(para)
	...
	def distance_metric(self, action_probs1, action_probs2):
		#print("Perturbed action is {}".format(action1))
		#print("Unperturbed action is {}".format(action2))
		#Compute distance between actions taken by two policies at the sane states
		#Expects data with structure of numpy arrays
		diff = action_probs1 - action_probs2
		mean_diff = np.mean(np.square(diff), axis = 0)
		dist = np.sqrt(np.mean(mean_diff))
		print("Action distance is {}".format(dist))
		return dist

	def remember(self, ob, action, reward, next_ob, done, action_probs):
		self.memory.reme(ob, action, reward, next_ob, done, action_probs)
	...
	def act_(self, observations_for_agent, episode):
		...
		for agent_id in self.agent_list:
			...
			actions_probs[agent_id] = action_probs
	...
	def act(self, obs, episode):
		...
		for agent in self.agent_list:
			actions[agent], actions_probs[agent], entropys[agent] = self.get_action(observations_for_agent[agent]['lane_vehicle_num'], is_param, is_ou, is_explore) + 1

		return actions, actions_probs, entropys

	def get_action(self, ob, is_param = False, is_ou = False, is_explore = True):
		#If need param noise
		#print("Ob is {}".format(ob))
		if is_param:
		...
		entropy = -tf.reduce_mean(tf.reduce_sum(tf.math.log(action) * action))
		#If need sampling rather than simply argmax
		if is_explore:
			#归一化后取样
			action[action < 0] = 0 #把<0的action替换为0，可以作为概率数组使用
