...
def load_agent_submission(submission_dir: Path):
    ...
    for dirpath, dirnames, file_names in os.walk(submission_dir):
        for file_name in [f for f in file_names if f.endswith(".py")]:
            if file_name == "agent_3DQN_PSER.py":
                module_path = dirpath
    ...
    ), "Cannot find file named agent_3DQN_PSER.py, please check your submission zip"
    assert(
    ...
    import agent_3DQN_PSER as agent_submission
...
#os.mknod("test.txt")
#rewards_double_dqn_PRE_360 = os.mknod("rewards_double_dqn_PRE_360.txt") 
        ...
        while i < args.steps:
            if i % args.action_interval == 0:
                ...
                for key, val in observations.items():
                    #
                    #print("Key in observations items is {}".format(key))
                    #print("Val in observations items is {}".format(val))
                ...
                for _ in range(args.action_interval):
                    ...
                    '''for agent_id in agent_id_list:
                        ...
                        #
                        print("Lane_vehicle is {}".format(lane_vehicle))
                    ...
                    for agent_id in agent_id_list:
                        lane_vehicle = observations["{}_lane_vehicle_num".format(agent_id)]
                        lane_speed = observations["{}_lane_speed".format(agent_id)]
                        queue_length = 0
                        for i in range(1, 13):
                            if lane_vehicle[i] > 0 and lane_speed[i] >= 0 and lane_speed[i] < 0.1:
                                queue_length += lane_vehicle[i]
                        #
                        #print("queue_length for agent_id {} is {}".format(agent_id, queue_length))
                        avg_queue_length = queue_length / args.action_interval
                        #queue length应该是作为惩罚存在，是否需要累加？
                        if agent_id in rewards_list:
                            rewards_list[agent_id] -= avg_queue_length
                        else:
                            rewards_list[agent_id] = -avg_queue_length
                ...
                ##
                ##print("Average rewards for episode {} {}th simulation is {}".format(args.episodes, i, sum(rewards) / len(rewards)))
                ##with open("rewards_dqn_pre_360.txt", "a") as r_dqn:
                    ##r_dqn.write("Average rewards for episode {} {}th simulation is {}\n".format(args.episodes, i, sum(rewards) / len(rewards)))
                ##r_dqn.close()
                ##
