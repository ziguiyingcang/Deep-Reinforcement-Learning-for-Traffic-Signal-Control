...
class ActorNetwork(object):
	def __init__(self, sess, ob_length, action_space, layer1_size, learning_rate, tau):
		self.sess = sess
	...
	def create_training_method(self):
		self.q_gradient_input = tf.placeholder(tf.float32, [None, self.action_space]) #input from critic network
	...
	def create_network(self, ob_length, action_space):
		...
		action_output = tf.nn.softmax(tf.matmul(layer1, W2) + b2)
	...
	def create_target_network(self, ob_length, action_space):
		...
		action_output = tf.nn.softmax(tf.matmul(layer1, target_net[2]) + target_net[3])
...
class CriticNetwork(object):
	def __init__(self, sess, ob_length, action_space, layer1_size, c_learning_rate, l2, c_tau):
		self.sess = sess
		self.layer1_size = layer1_size
...
class DDPGAgent():
	def __init__(self):
		...
		self.actor_network = ActorNetwork(self.sess, self.ob_length, self.action_space, self.layer1_size, self.a_learning_rate, self.a_tau)
		self.critic_network = CriticNetwork(self.sess, self.ob_length, self.action_space, self.layer1_size, self.c_learning_rate, self.l2, self.c_tau)
...
#如果是连续动作，且动作为（相位序数，持续时间）这个形式，这里如何定义选取动作？
	def act_(self, observations_for_agent, episode):
		...
		actions = {}
		if episode < self.explore_max_ep:
			chooseFrom = np.arange(0, self.action_space)
			for agent_id in self.agent_list:
				action = self.get_explore_action(observations_for_agent[agent_id]['lane'], chooseFrom)
				actions[agent_id] = action
				#print("Action for agent_id {} is {}".format(agent_id, action))
		else:
			for agent_id in self.agent_list:
	...
	def act(self, obs, episode):
		...
		if episode < self.explore_max_ep:
			...
			for agent in self.agent_list:
				actions[agent] = self.get_explore_action(observations_for_agent[agent]['lane_vehicle_num'], chooseFrom) + 1
		else:
			for agent in self.agent_list:
	...
	def get_explore_action(self, ob, chooseFrom):
		#Select action according to the current policy and exploration noise
		action = self.actor_network.action(ob)
		a = np.random.choice(chooseFrom, size = 1, p = action)
		a = a[0]
		return a
