...
def load_agent_submission(submission_dir: Path):
    ...
    for dirpath, dirnames, file_names in os.walk(submission_dir):
        for file_name in [f for f in file_names if f.endswith(".py")]:
            if file_name == "agent_DDPG_discrete.py":
                module_path = dirpath
    ...
    ), "Cannot find file named agent_DDPG_discrete.py, please check your submission zip"
    assert(
    ...
    import agent_DDPG_discrete as agent_submission
...
def train(agent_spec, simulator_cfg_file, gym_cfg, metric_period):
    ...
    for e in range(args.episodes):
        ...
        i = 0
        ##
        #print("Args steps is {}".format(args.steps))
        #print("Args action interval is {}".format(args.action_interval))
        ##
        while i < args.steps:
            ##
            #print("Step {}".format(i))
            ...
            if i % args.action_interval == 0:
                ...
                ##
                    #print("{}th episode, {}th simulation, ob_agent_id is {}, ob_feature is {}, val is {}/n".format(e, i, #observations_agent_id, observations_feature, val))
                ##
                # Get the action, note that we use act_() for training.
                actions, actions_probs, entropys = agent.act_(observations_for_agent, e)
                ...
                for _ in range(args.action_interval):
                    ...
                    for agent_id in agent_id_list:
                        ...
                        ##
                        ##print("Pressure for {}th agent {}th episode {}th simulation {}th action is {}".format(agent_id, e, i, _, pressure))
                        ##with open("pressure_drqn.txt", "a") as r_dqn:
                        	##r_dqn.write("Pressure for {}th agent {}th episode {}th simulation {}th action is {}\n".format(agent_id, e, i, _, pressure))
                        ##r_dqn.close()
                        ##
                ...
                for agent_id in agent_id_list:
                    agent.remember(observations_for_agent[agent_id]['lane'], actions[agent_id], rewards[agent_id],
                                   new_observations_for_agent[agent_id]['lane'], dones[agent_id], actions_probs[agent_id])
                    #agent.remember(observations_for_agent[agent_id]['lane'], actions[agent_id], rewards[agent_id],
            ...
            #Do perturb actor parameter
            if e < agent.para_noise_max_ep and total_decision_num % agent.update_param_fq == agent.update_param_fq - 1:
                agent.perturb_actor_parameters()

            #Update param noise stddev
            if e < agent.para_noise_max_ep and total_decision_num % agent.update_param_fq == agent.update_param_fq - 1:
                current_memory = agent.memory.memory
                current_size = agent.memory.num_experiences
                update_param_batch = current_memory[(current_size-agent.update_param_fq):current_size]
                obs = np.asarray([data[0] for data in update_param_batch])

                unperturbed_actions = list()
                for ob in obs:
                    action, action_probs, entropy = agent.get_action(ob, False, False, True)
                    #print("Action_probs is {}".format(action_probs))
                    unperturbed_actions.append(action_probs)
                unperturbed_actions = np.asarray(unperturbed_actions)
                perturbed_actions = np.asarray([data[5] for data in update_param_batch])

                dist = agent.distance_metric(perturbed_actions, unperturbed_actions)
                agent.ap_explore.adapt(dist)

            if all(dones.values()):
                agent.ou_explore.reset()
                break
...
def run_simulation(agent_spec, simulator_cfg_file, gym_cfg,metric_period,scores_dir,threshold):
    ...
    while not done:
        ...
        actions, actions_probs, entropys = agent.act(all_info, step)
